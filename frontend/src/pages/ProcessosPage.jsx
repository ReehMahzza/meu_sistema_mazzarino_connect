// frontend/src/pages/ProcessosPage.jsx
...
function ProcessosPage() {
    ...
    useEffect(() => {
        const fetchProcessos = async () => {
            try {
                setLoading(true);
                setError('');
                const response = await axiosInstance.get('/api/cases/?case_type=resolucao_conflitos_telecom');
                setProcessos(response.data);
            } catch (err) {
                setError("Falha ao carregar a lista de processos.");
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) { return <p className="text-center text-gray-500">Carregando processos...</p>; }
    if (error) { return <p className="text-center text-red-500 bg-red-100 p-4 rounded-md">{error}</p>; }

    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-gray-800">Processos Administrativos</h1>
                <Link to="/casos/novo" state={{ defaultCaseType: 'resolucao_conflitos_telecom' }}>
